...
class ResourceAllocationRow(BaseModel):
    ...
    risk_level: str = "High"
    population: int = 0
    ambulances_requested: int = 0
    ambulances_allocated: int = 0
    boats_requested: int = 0
    boats_allocated: int = 0
    medics_requested: int = 0
    medics_allocated: int = 0
    food_packs_allocated: int = 0
    tradeoff_rationale: str = ""

class AgentConflictResolution(BaseModel):
    id: str
    agents_involved: List[str]
    contested_resource: str
    conflict_description: str
    resolution_strategy: str
    tradeoff_penalty: str
    status: str = "RESOLVED"

class PublicAlertDraft(BaseModel):
    headline_en: str
    headline_ta: str
    body_en: str
    body_ta: str
    evacuation_routes: List[str]
    safe_shelters: List[str]
    helpline: str
    urgency: str = "Immediate / Evacuation"
    approved_by_commander: bool = False
...
class CoordinatorDashboardResponse(BaseModel):
    ...
    allocation_table: List[ResourceAllocationRow] = Field(default_factory=list)
    conflict_resolutions: List[AgentConflictResolution] = Field(default_factory=list)
    public_alert_draft: Optional[PublicAlertDraft] = None
